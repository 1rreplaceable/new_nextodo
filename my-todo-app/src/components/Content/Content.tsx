        ...
        <main className="flex-1 bg-white p-6">
            ...
            {isModalOpen && selectedTodo && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white w-1/2 p-8 rounded-lg shadow-lg relative max-h-fit overflow-y-auto">
                        <button
                        ...
                        <h2 className="text-2xl font-bold mb-6 border-b pb-2">{selectedTodo.title}</h2>
                        <div className="mb-4">
                            <p className="mb-2"><strong>완료 여부:</strong> {getStatusLabel(selectedTodo)}</p>
                            <p className="mb-2"><strong>시작일:</strong> {selectedTodo.startDate ? new Date(selectedTodo.startDate).toLocaleDateString() : "미정"}</p>
                            <p className="mb-2"><strong>종료일:</strong> {selectedTodo.endDate ? new Date(selectedTodo.endDate).toLocaleDateString() : "미정"}</p>
                        </div>
                        {/* 댓글 섹션 */}
                        <div className="mt-6">
                            ...
                            <ul className="space-y-4">
                                {comments.length > 0 ? (
                                    comments.map((comment) => (
                                        <li key={comment.todoId} className="flex items-start space-x-4 relative group">
                                            ...
                                            <div className="bg-gray-100 rounded-lg p-4 flex-1">
                                                <p className="font-semibold text-blue-600 mb-1">{comment.writer}</p>
                                                <p>{comment.text}</p>
                                            </div>
                                            {userName === comment.writer && (
                                                <button
                                                    className="absolute font-semibold top-2 right-4 text-red-500 group-hover:block"
                                                    // onClick={(e) => {
                                                    //     e.stopPropagation();
                                                    //     handleDeleteComment(comment.todoId);
                                                    // }}
                                                >
                                                    &#10005;
                                                </button>
                                    ...
                                    <p className="text-gray-500">댓글이 없습니다. 첫 댓글을 작성해 보세요!</p>
                                )}
                            </ul>
                        </div>
                        {/* 댓글 작성 폼 */}
